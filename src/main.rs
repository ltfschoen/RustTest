...
// Import the Crate libc, which are bindings for the C standard library
// for use with a FFI (Foreign Function Interface) that allows writing bindings
// from foreign code by calling into the C Interface of a compression/decompression
...
// Import from Custom C Library using FFI and gcc Crate Library
fn try_ffi_custom() {
    #[link(name="hello", kind="static")]

    extern { fn hello(); }

    let test1 = unsafe { hello(); };

    println!("Output of running test1: {:?}", test1);
